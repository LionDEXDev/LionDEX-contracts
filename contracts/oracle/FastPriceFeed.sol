...
contract FastPriceFeed is ISecondaryPriceFeed, IFastPriceFeed, Governable {
    ...
    uint256 public priceDuration; //5min = 300
    uint256 public maxPriceUpdateDelay;//1hour =3600
    uint256 public spreadBasisPointsIfInactive;//0.2% = 20
    uint256 public spreadBasisPointsIfChainError;//5% = 500
    uint256 public minBlockInterval;//0
    uint256 public maxTimeDeviation;// 3600

    uint256 public priceDataInterval; //1min =60

    // allowed deviation from primary price
    uint256 public maxDeviationBasisPoints;//oracle fast  10% = 1000

    uint256 public minAuthorizations;//1
    ...
    mapping(address => uint256) public maxCumulativeDeltaDiffs;//WBTC,eth,uni,link=1000000
    ...
    constructor(
        ...
        _positionRouter = _positionRouter;
    }
    ...
    function getPrice(address _token, uint256 _refPrice, bool _maximise) external override view returns (uint256) {
        if(_refPrice ==0 ){ //onlytest   need to delete on main net 
